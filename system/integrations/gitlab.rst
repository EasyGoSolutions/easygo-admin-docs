...
Use GitLab integration to track GitLab issues directly within EasyGo Solutions tickets.
...
1. Create a new API token in GitLab. EasyGo Solutions supports the usage of a
...
3. Enter your new API token in EasyGo Solutions and enable the GitLab integration.
...
Token verification is taking a long time
   ...
   to ensure that your EasyGo Solutions server can reach ``gitlab.com``.

I reloaded the page and now the API token is gone
   This may indicate that EasyGo Solutions is still verifying your API token.
