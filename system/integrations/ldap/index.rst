...
EasyGo Solutions comes with a powerful LDAP integration that allows you to have a single
source of truth. Using this integration reduces the number of login credentials
your users have to remember.

.. hint::

   The LDAP source is also a perfect candidate for EasyGo Solutions's
...
.. figure:: /images/system/integrations/ldap/ldap-integration-management.png
   :width: 90%
   :align: center
   :alt: Screenshot shows EasyGo Solutions's LDAP settings.
...
- EasyGo Solutions's LDAP sync is one way. Edited user settings or permissions may be
...
- When a user originates from an LDAP server, EasyGo Solutions will try to verify the
  login credentials against LDAP first - if this fails EasyGo Solutions will check its
  local database.
...
- When several LDAP sources contain the same user (meaning the same email
  ...
  `Issue 4109 <https://github.com/EasyGo Solutions/EasyGo Solutions/issues/4109>`_ for more
  details.
- Synchronization statistics currently affect *all* configured LDAP sources.
  ...
  `Issue 4108 <https://github.com/EasyGo Solutions/EasyGo Solutions/issues/4108>`_ for more
  details.
- EasyGo Solutions currently has limited fallback server support. You can workaround
  this by providing several sources - however, ensure to have the exact same
  configuration on your fallback. See
  `Issue 4107 <https://github.com/EasyGo Solutions/EasyGo Solutions/issues/4107>`_ for more
  details.
...
EasyGo Solutions will display an additional **SSL verification** option that allows you
to disable the verification, e.g. for self-signed SSL certificates.
You can also tell EasyGo Solutions to use a different port by
...
.. tip::
   ...
   of your LDAP users in EasyGo Solutions. Because Active Directories are quite
   ...
   - As every LDAP behaves differently on which attributes and how they are
     set, EasyGo Solutions doesn't care about any flags.
   - Users that no longer are returned by your LDAP source will
     automatically be set to inactive. EasyGo Solutions assumes that the user was
     deactivated.
...
.. danger::
   ...
   *Do not* manually write paths of either LDAP attributes or groups.
   If EasyGo Solutions does not display them, it either cannot find them or you have
   a lot of users that don't have the attributes populated.

   EasyGo Solutions will always only return attributes that are filled - this reduces
...
.. note::
   ...
   If your LDAP system doesn't allow anonymous bind, EasyGo Solutions detects it and
...
.. hint::
   ...
   EasyGo Solutions roles to all members of nested groups.
...
EasyGo Solutions will synchronize the sources from top to bottom. To change the order,
...
This section holds all requests EasyGo Solutions handled for all LDAP sources.
These entries can either contain synchronization information or logins
(authentication attempts via EasyGo Solutions's login interface).
By clicking on any request, EasyGo Solutions will provide even more information.
