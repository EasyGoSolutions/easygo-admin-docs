...
System notifications are automated emails sent by EasyGo Solutions for critical system
...
:doc:`/manage/scheduler`, these notifications are built into EasyGo Solutions itself:
...
Daily reminder emails are sent at midnight (of the EasyGo Solutions system timezone)
...
Inside your EasyGo Solutions directory (usually ``/opt/EasyGo Solutions``), email templates for
various events are stored inside the ``/app/views/mailer`` directory, named
according to the language they're written in. Thus,

.. code-block:: text

   /opt/EasyGo Solutions/app/views/mailer/ticket_create/de.html.erb
...
.. code-block:: text
   ...
   /opt/EasyGo Solutions/app/views/mailer/ticket_create/de.html.erb.custom
