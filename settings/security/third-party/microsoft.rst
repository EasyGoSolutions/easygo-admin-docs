...
EasyGo Solutions's Microsoft connection allows your users with Microsoft accounts to log
in. This works for Entra ID users as well and can be an alternative to
:doc:`/system/integrations/ldap/index`. The configuration in EasyGo Solutions is located
...
.. figure:: /images/settings/security/third-party/microsoft/EasyGo Solutions-login-interface-with-microsoft-login.png
   :alt: Screenshot shows Microsoft login button on EasyGo Solutions login screen.
   :align: center

Limitations
-----------

Please note that EasyGo Solutions only supports these account types (app-dependent):
...
Redirect URI (optional)
   ...
   and copy it from EasyGo Solutions's read-only **Your callback URL** field. Paste it
   into the redirect URI field in Microsoft's app configuration.

   The callback URL looks like this:
   ``https://EasyGo Solutions.example.com/auth/microsoft_office365/callback``
...
Add App Credentials to EasyGo Solutions
-----------------------------

Now you are ready to provide the values to EasyGo Solutions. The following table
maps the values you collected in Entra ID to the fields in EasyGo Solutions.
...
make EasyGo Solutions insist on that signal before it links a Microsoft login to an
existing EasyGo Solutions user by email address.

.. note::

   This option only has an effect in combination with
   :ref:`Automatic account link on initial logon <automatic-account-linking>`.
   Without it, EasyGo Solutions never links a third-party login to an existing account
   by email address in the first place.

With the option enabled, EasyGo Solutions links a login to an existing user only if all
...
If one of these conditions is not met, EasyGo Solutions refuses to link the accounts and
...
access: they log in to their existing EasyGo Solutions account by other means and link
their Microsoft account themselves under *Profile > Linked Accounts*.

While the option is disabled (default), EasyGo Solutions does not require the claim:
